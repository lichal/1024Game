...
public class TenTwentyFourTester {
    ...
    public void resetShouldShowTwoValue()
    {
        ...
        assertTrue (c1.getRow() != c2.getRow() || c1.getCol() != c2.getCol());
    ...
    private void swiper (NumberSlider cm, SlideDirection dir, String chPat, int[][] before, int[][] after)
    {
        ...
        for (Cell c : out) {
            if (c.getRow() < 0 || c.getRow() >= after.length || c.getCol() < 0 || c.getCol() >= after[0].length)
                fail("game1024.Cell index (" + c.getRow() + "," + c.getCol() + " is out of bound");
            assertTrue("getNonEmptyTiles() should return only non-zero cells", c.getValue() != 0);
            actual[c.getRow()][c.getCol()] = Math.abs(c.getValue());
        }
    ...
    private void noMoveSwiper (NumberSlider cm, SlideDirection dir, int[][] mat)
    {
        ...
        for (Cell c : out) {
            if (c.getRow() < 0 || c.getRow() >= mat.length || c.getCol() < 0 || c.getCol() >= mat[0].length)
                fail("game1024.Cell index (" + c.getRow() + "," + c.getCol() + " is out of bound");
            assertTrue("getNonEmptyTiles() should return only non-zero cells", c.getValue() > 0);
//            assertTrue(action + " (" + c.row + "," + c.column + ") should be zero",
//                    after[c.row][c.column] != 0);
            actual[c.getRow()][c.getCol()] = c.getValue();
        }
